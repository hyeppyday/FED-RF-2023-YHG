...
// Multer 미들 웨어를 불러온다! 왜? 파일 전송 처리를 위해!
...
// 폴더가 없을 경우 폴더를 생성해줌!
// 하지만 파일명이 겹쳐지므로 이것을 커스터마이징 하여
// 변경하게 되면 자동으로 폴더를 생성해 주지 않아서 파일이 올라오지 않는다 ㅠㅠ
// 그렇기 때문에 우리가 직접 uploads라는 폴더를 생성해줘야한다
// 배포시 이 폴더가 생기도록 SPA 개발 폴더의 public 아래에
// uploads 폴더를 만들어준다!

// 파일명을 내가 원하는 원래 이름으로 들어가도록 변경한다!
// 멀터 스토리지의 저장소를 사용한다
const storage = multer.diskStorage({
    // 폴더경로를 여기 설정함 (dest 설정은 지워준다!)
...
// 파일 업로드는 POST 방식으로 진행함!
// 익스프레스 서버 메서드에 post() 메서드로 설정함!
// -> 첫번째 값은 루트 아래에 업로드 관련 post 전송을 선택
// -> upload는 폴더명이 아니라 작업명이다!
// -> 두번째 값은 전송종류를 설정 : 파일전송은 'file'
// -> 세번째 값은 내부 전달 변수인 요청, 응답에 대한 함수
...
// 서버 루트폴더 정적연경하기!(루트정하기)
// -> SPA에서 빌드하면 배포용 소스가 build폴더에 생성되므로
// 이 배포용 폴더를 Root로 잡으면 편하다!
app.use(express.static(path.join(__dirname, "/build")));
// -> SPA앱 빌드시 유의사항 : package.json 파일에
// home:'http://localhost:8080'를 등록하여 사용함!!!!
// localhost는 127.0.0.1 내부 호출 아이피와 동일하다!

// 첫페이지 설정하기 -> url로 쳐서 들어가는 경로를 설정!
