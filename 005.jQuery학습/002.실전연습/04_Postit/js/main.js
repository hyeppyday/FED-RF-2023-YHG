...
$(()=>{
    ...
    $('.dropshow').droppable({
        drop: function(evt,ele){
            ...
            // 4. 드롭된 요소의 자식 p태그 글자 읽어오기
            let ptxt = target.find('p').text();
            console.log('텍스트:',ptxt)
            ...
            // html()로 넣으면 닫기버튼 지워져서
            // 맨뒤에 요소 추가는 append() 메서드 사용하기!
            ...
            // 영상 주소 읽어오기
            let dmv = target.attr('data-mv');
            console.log("주소:",dmv)


            mbox.append(`
            <iframe src="https://www.youtube.com/embed/${dmv}?autoplay=1" allow="autoplay" style="width:100%;height:100%;border:none"></iframe>
            `)
            ...
            .css({
                ...
                mbox.slideUp(1000,function(){
                    ...
                    $(this).remove(); // 태그제거 (안할시에 소리는 계속 나게됨)
