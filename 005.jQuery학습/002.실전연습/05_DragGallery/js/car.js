...
$(() => {
  ...
  /***************************************** 
        ...
        원리 : 마우스 포인터 위치 중 x 축값만 이용하여
        처음찍은 위치와 드래그하여 마지막에 찍은 위치를
        비교하여 방향을 결정 한 후 이전/다음 이미지로
        순서태로 넘겨서 자동차를 보여준다!
    *****************************************/
  ...
  const rotateCar = dir => { // dir = 방향
    ...
    if(dir){
        fnum++;
        if(fnum==50) fnum = 0;
        // 마지막 순번은 49번 이므로 50번에서 0으로 변경해줌
    } //////////////// if ///////////////////
    ...
    // (2) 이미지 순번 감소 처리
        ...
        fnum--;
        if(fnum==-1) fnum = 49;
        // 첫순번은 0이므로 -1이면 마지막 순번 49번으로 변경해줌
    } ////////////////// else ///////////////////
    console.log('순번:',fnum);
