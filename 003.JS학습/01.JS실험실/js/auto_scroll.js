...
/******************************************************************** 
    ...
    -> wheel 이벤트
    -> 마우스 휠 작동시 발생!
    (이전 이벤트명 : mousewheel / DOMMouseScroll(fire fox))
********************************************************************/
...
function loadFn(){
    ...
    // 전체 페이지 수 할당
    total_pg = domFn.qsa('.page').length;
...
/**************************************************************** 
    ...
    기능 : 마우스 휠 작동시 페이지이동
****************************************************************/
function wheelFn(e){ // 이벤트 전달변수(자동)
    // 함수 호출확인
    //console.log('휠~~~~');
...
// 2-1. 터치 시작 이벤트 호출함수
    ...
    function touchEnd(e){ // e - 이벤트 전달변수
        ...
        // 제이쿼리는 originalEvent를 사용해야 나옴!!
        // let scY = e.originalEvent.touches[0].screenY;
        // 터치 끝낼때는 changedTouches[0] 사용하기 !
        pos_end = e.changedTouches[0].screenY;
    
        // 2. 터치방향 알아내기 /////////////////
        // 원리 : 시작위치 - 끝 위치
        // 음수면 위에서아래로 터치 양수면 아래에서위로 터치
...
// 2-3. 이벤트 처리함수 : 화면이동
function movePage(dir){ // dir은 방향값(1은 아래쪽 0은 위쪽)
    //console.log('이동방향은?',dir);

    //1. 페이지번호 변경 반영하기


    // 1은(true) 아랫방향, 0은(false) 윗방향
    // 1값은 true, 0값은 flase로 처리됨!
    if(dir) pg_num++;
    else pg_num--;
