...
function localSFn(){
    ...
    if(btxt == '처음'){
        ...
        // 해당 url로 관리되는 로컬스토리지를 모두 지움 : clear()
        localStorage.clear();
        // 개별 로컬스토리지를 지우는 방법은 removeItem(키명)
    } ////////////////// else if : 전체삭제 /////////////////////
    else if(btxt == '보여줘'){
    ...
    function makeObj(){
        ...
        // -> 왜? 로컬스토리지는 문자형만 받을 수 있기때문
        // 그래서 배열/객체를 문자데이터화 해서 넣는다!!!
        // JSON.stringgify(배열/객체)
...
function bindData(){
    ...
    if(localData){ // null이 아니면 true!
        ...
        // 문자형을 배열로 형변환해야함!!!
        // 로컬스토리지 데이터 배열 객체형 변환 -> JSON.parse(문자형배열객체)
...
function sessionSFn(){
    // 1. 버튼텍스트 읽기
    let btxt = this.innerText;
    console.log('세션쓰:',this);
    // 2. 버튼별 기능 분기하기
    if(btxt == '처음'){
        // 세션 스토리지 읽기 : sessionStorage.getItem(키명)
        // console.log('세션쓰 lname:',sessionStorage.getItem('lname'));
        // 만약 값이 셋팅 안됐으면 null값이 나옴
        // 세션스토리지 셋팅 : sessionStorage.setItem(키명,값)
        sessionStorage.setItem('lname','정우성')
        sessionStorage.setItem('lrole','김정도역')
        sessionStorage.setItem('lcat','국내팀 안기부팀장, 박평호와 사이가 안좋음')

        // console.log('세션쓰 lname:',sessionStorage.getItem('lname'));

    } //////////////// if : 처음 /////////////////////
    else if(btxt == '전체삭제'){
        // 해당 url로 관리되는 세션스토리지를 모두 지움 : clear()
        sessionStorage.clear();
        // 개별 세션스토리지를 지우는 방법은 removeItem(키명)
    } ////////////////// else if : 전체삭제 /////////////////////
    else if(btxt == '보여줘'){
