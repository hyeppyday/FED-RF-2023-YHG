...
function showTime(){
    ...
    // 참고 : getYear()는 현재에서 1900년을 뺀 년도가 나옴
    ...
    // H = 3;
    // H = 13;
    tt[5].innerText = H>12?addZero(H-12):addZero(H);
...
// 1. 요구사항 : 웹경로 이미지를 화면에 넣고 랜덤하게 이미지를 칼라로 약간 커지게 클래스 on을 주어서 변경하기
...
// 직전 랜덤수 : 초기셋팅은 0 ~ 3 사이 수가 아닌걸로(내가 만든 배열수가 아닌걸로)
let bNum = 1000;
...
function colorImg(){
    ...
    // 2. 직전 랜덤수가 반복될 경우 다시 난수 발생 시켜주기
    // while(조건true){코드} 사용!!
