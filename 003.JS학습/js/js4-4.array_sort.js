...
/***************************************************** 
    ...
    [ sort() 메서드의 특징 ]
            ...
            "25"와 "100" 중 큰숫자는 100이지만 25를 더 큰 데이터로 인식한다
            -> sort() 메서드 비교함수로 처리!
    ...
    [ sort() 메서드 비교함수 ]
            ...
            숫자데이터배열.sort(function(a,b)=>a-b) : 오름차순
            
            숫자데이터배열.sort(function(a,b){return b-a;}) : 내림차순
            숫자데이터배열.sort(function(a,b)=>b-a) : 내림차순
            -> a는 앞데이터 b는 뒷데이터
    ...
    // 숫자형 문자형에 무관하게 하나로 처리하는 방법 ///////////////////////////////////
    -> 크기 비교로 사용하는데, 문자는 '사전순서'대로 크기 비교함
...
*************************************************************
    ...
    *************************************************
    ...
    [ 객체를 배열로 변환하여 리스트 만들기 : 정렬시 필수! 오브젝트.키스.맵]
...
// 문자값 배열
const arrString = ["파", "타", "하", "가", "바", "사", "다", "라", "차", "나"];

// sort()는 기본문자로 처리하므로 숫자는 내부함수로 빼기 연산처리함 !
...
function sortingFn() {
  ...
  console.log('앞에누구?',this.previousElementSibling); // 함수의 추상화 작업
  // this -> 콤보박스 자신
  // 앞에있는 형제요소 선택 : this.previousElementSibling
  // 뒤에있는 형제요소 선택 : this.nextElementSibling

  // 2. 정렬 기준값 읽기 : idx, tit, cont
  let cta = cta3.value;
...
// (3) 리스트 초기호출!!
// 위의 upCode()로 함수를 호출하여 페이지 찍기
upCode(list2, showList4);



// (4) sel4이벤트 설정하기
// 데이터와 출력 타겟부터 설정 후 정렬함수 호출!! -> 순서대로 실행되기 때문
dFn.addEvt(sel4, "change", ()=>{
  // 정렬용 데이터는 원본 list2를 쓰지않고 newList 사용
  targetData = newList;
  targetEle = showList4;
}); ///////////// change //////////////
...
function searchingFn(){
  ...
  let res = list2.filter(v=>{
    // v[객체속성명] -> cta 변수에 tit,idx,cont 중 하나가 들어옴 
    // (선택된 것이 들어올 수 있게 cta변수에 담아놓음)
    // indexOf(검색어) -> stxt
  ...
  // 그 변수의 값을 업데이트 하기 (원본데이터는 수정 불가)
  // 단, 변수 데이터는 정렬시에 사용하도록 한다
  // -> newList 변수
  newList = res;
