...
// console.log(mTitle,sTitle,personInfo,mvData,domFn);
// console.log(zzz('헐'));
// console.log(dFn);
console.log(msgFn);
...
// 1. 대상선정 : 출력박스
// (1) 타이틀 출력박스
const titBox = domFn.qs(".tpart");
// (2) 내용출력박스
const contBox = domFn.qs('#demo')
// (3) 영화정보 출력박스
const mvBox = domFn.qs('.mvpart');
...
// 이름과 나이가 셋팅된 배열데이터를 이용해 출력하기
pInfo.forEach(val=>contBox.innerHTML+=msgFn(val[0],val[1]));

// (3) 영화정보 뿌리기
// ol>li 형식으로 .mvpart 박스에 영화정보를 구성함
// 데이터는 mv변수
mvBox.innerHTML = '<h3>♥영화 위시리스트♥</h3>'
mv.forEach(val=>{mvBox.innerHTML+=`
<ol>
    <li>★제목:${val[0]}</li>
    <li>★장르:${val[1]}</li>
    <li>★감독:${val[2]}</li>
